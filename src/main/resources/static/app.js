

//local player data
var mycar=undefined;
var mycarxpos=10;
var mycarypos=10;


//competitors data
var loadedCars=[];
var carsCurrentXPositions=[];
var carsCurrentYPositions=[];
var racenum = 1;

var stompClient = null;

movex = function(){    
    mycarxpos+=10;
    paintCars();
    stompClient.send("/topic/car"+mycar.number, {}, JSON.stringify({car:mycar.number,xpos:mycarxpos}));
    if(mycarxpos >= 640){
        let recentrace = racenum-1;
        console.log(mycar);
        $.ajax({
                 url: "races/"+recentrace+"/winner",
                 type: 'POST',
                 data: JSON.stringify(mycar),
                 contentType: "application/json"
             }).then(
                     function(data){
                         alert("There is a winner!");
                         stompClient.send("/topic/winner", {}, JSON.stringify({number:data}));
                         _setWinner(data);
                     },
                     function(err){
                         alert("err:"+err.responseText);
                     }
                   );
    }
};

initAndRegisterInServer = function(){
    mycar={number:$("#playerid").val()};
    mycarxpos=10;
    mycarypos=10;
    
    $.ajax({
        url: "races/"+racenum+"/participants",
        type: 'PUT',
        data: JSON.stringify(mycar),
        contentType: "application/json"
    }).then(
            function(){
                alert("Competitor registered successfully!");
                loadCompetitorsFromServer();
                paintCars();
            },
            function(err){
                alert("err:"+err.responseText);
            }
                 
    );
    $('#racenum').html(racenum);
};

init = () =>{
    document.getElementById("movebutton").disabled = true;
    document.getElementById("movebutton").style.background='#9b9b9b';
    $('#racenum').html(racenum);
}

_setWinner = (winner) => {
    $('#winner').html(msgdata.number);
}

const loadCompetition() = new Promise((resolve, reject) => {
    if(loadedCars.length == 5){
        resolve(true);
    }else{
        reject();
    }
})

loadCompetitorsFromServer = function () {
    if (mycar == undefined) {
        alert('Register your car first!');
    } else {
        $.get("races/"+racenum+"/participants",
                function (data) {
                    loadedCars = data;
                    var carCount = 1;
                    //alert("Competitors loaded!");

                    if (loadedCars.length == 5){
                       /* racenum++;
                        document.getElementById("movebutton").disabled = false;
                        document.getElementById("movebutton").style.background = '#4CAF50';*/
                        alert("Competitors loaded!");
                        connectAndSubscribeToCompetitors();
                    }

                    loadedCars.forEach(
                            function (car) {
                                if (car.number != mycar.number) {
                                    carsCurrentXPositions[car.number] = 10;
                                    carsCurrentYPositions[car.number] = 40 * carCount;
                                    carCount++;
                                }
                            }
                    );
                    loadCompetition
                        .then(data => {
                            stompClient.send("/topic/loaded", {} , 1);
                            console.log(data);
                        });
                }
        );
    }

};


function connectAndSubscribeToCompetitors() {
    var socket = new SockJS('/stompendpoint');
    stompClient = Stomp.over(socket);
    stompClient.connect({}, function (frame) {
        console.log('Connected: ' + frame);

        loadedCars.forEach(
                
                function (car) {
                    //don't load my own car
                    if (car.number!=mycar.number){
                        stompClient.subscribe('/topic/car'+car.number, function (data) {
                            msgdata=JSON.parse(data.body);
                            carsCurrentXPositions[msgdata.car]=msgdata.xpos;
                            paintCars();
                        });
                    }
                }
        );
        stompClient.subscribe('/topic/winner', function(data){
            msgdata=JSON.parse(data.body);
            _setWinner(msgdata.number);
        });
        stompClient.subscribe('/topic/loaded',function(data){
            //paintCars();
            loadCompetitorsFromServer();
            document.getElementById("movebutton").disabled = false;
            document.getElementById("movebutton").style.background = '#4CAF50';
        });
        
    });
}

function disconnect() {
    if (stompClient != null) {
        stompClient.disconnect();
    }
    setConnected(false);
    console.log("Disconnected");
}

paintCars=function(){
    canvas=$("#cnv")[0];
    ctx=canvas.getContext('2d');
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    //paint my car
    paint(mycar,mycarxpos,mycarypos,ctx);
    
    //paint competitors cars
    loadedCars.forEach(
            function(car){
                paint(car,carsCurrentXPositions[car.number],carsCurrentYPositions[car.number],ctx);
            }
    );
};


paint=function(car,xposition,yposition,ctx){
    var img = new Image;
    img.src = "img/car.png";
    img.onload = function(){        
        ctx.drawImage(img,xposition,yposition); 
        ctx.fillStyle = "white";
        ctx.font = "bold 16px Arial";
        ctx.fillText(""+car.number, xposition+(img.naturalWidth/2), yposition+(img.naturalHeight/2));        
    };    
};



$(document).ready(
        
        function () {
            console.info('loading script!...');
            $(".controls").prop('disabled', false);    
            $("#racenum").prop('disabled', true);
        }
);

